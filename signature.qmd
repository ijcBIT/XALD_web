...
```{r top_probes_contrast, fig.height=72, dpi=200}
#| eval: false
#| include: false
#| column: page
#| layout-ncol: 1
...
dt_mean <- dtp[,list(b=mean(bVal),avage=mean(Age)),by=c("Condition","model","cg")]
p_Contrasts<-ggplot(dtp, aes(x = Age, y = bVal, color=Condition, group=factor(model) )) +
    geom_point() +
    ylim(0, 1) +
    ...
    labs(title=paste0('Beta values vs. Age: top 10 sites by Contrast '), x='Age', y='Residuals') +
      facet_grid(cg ~ Contrast, margins = F)
p_Contrasts


# plts<-lapply(
#   unique(topDMPs$probe),
#   function(ProbeID){
#     dt_mean <- dt_plot[ProbeID,list(b=mean(bVal),avage=mean(Age)),by=c("Condition","model")]
#     
#     print(ProbeID)
#     p<-ggplot(dt_plot[ProbeID,], aes(x = Age, y = bVal, color=Condition, group=factor(model) )) +
#     geom_point() +
#     ylim(0, 1) +
#     geom_point(data=dt_mean,aes(x=avage,y=b,col=Condition),size=4,shape=24)  +
#     geom_smooth(aes(group=Condition),
#               method = "lm", formula = y ~ x, se = F) +
# 
#     labs(title=paste0('Residual vs. Age for: ',ProbeID), x='Age', y='Residuals') + 
#       facet_grid(vars(cg))
#     
#     return(p)
#     
#   })
# sapply(plts,function(x)plot(x))
# # p<-do.call("grid.arrange",c(plts, ncol=2))
# # grid.arrange(grobs = plts, ncol = 2) ## display plot
# plot(p)
```
...
We see there is a small difference in methylation for the dataset built with our controls and also for the dataset from literature.
