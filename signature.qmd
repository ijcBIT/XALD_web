...
# Question 1: Aging signature in healthy controls (children vs adults): {#sec-intro}
...
As the difference between XALD age groups where much less then those observed in previous studies, we set out to confirm the existence of an age signature (age associated methylation changes) in our control samples following the workflow previously described in Steegena et al. @Steegenga2014 Briefly,

1.  we perform a comparison between children and adult controls
2.  we check for methylation changes at known age-associated sites.

## Reference aging signature: {#sec-PBMC}
...
![density_plot](%60r%20paste0(params$ctl_qc_dir,%22density_plot.png%22)%60){alt="density_plot"}

![bean_plot](%60r%20paste0(params$ctl_qc_dir,%22bean_plot.png%22)%60){alt="bean_plot"}
...
Following @Steegenga2014 we normalize the data using the subset-quantile within array normalization (SWAN) method:
...
As reference set we will use age-reated sites described in the paper by Wilma, T. et al @Steegenga2014 . Where they study the relationship between gene expression and DNA methylation changes in Peripherial Blood Mononuclear Cells (PBMC) cells

The authors establish 3 different datasets of CG sites:
...
3.  cgSet_pre: Probes reported to be age-related in previous literature (literature-curated).

We created an additional one by combining 2 &3: 4. cgSetm: The combination/union of cgSet_Exprs & cgSet_NO

We process our data according to their workflow
...
To analyse DMPs in age-related sites in the disease, we will use a combination of these reference sets, as well as sub-samples corresponding to the intersections between DMPs from our controls and the reference sets. For more about inhouse reference datasets @sec-inhouseref .
